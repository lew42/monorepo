...
- 2026-09-01 (padding-study minion): typed sequential timestamps by mental estimate across a long tool-heavy stretch (many playwright runs between log lines) instead of calling `date -Iseconds` before each append — they drifted ~15 min AHEAD of the real clock, caught only when the finish-task landing-line step forced a fresh `date` call. The warning is already in the skill; what would have caught it sooner is calling `date -Iseconds` as its own bash call immediately before composing each log line's JSON, not batching "write several jsonl lines" as one mental pass.

...
const rail = (page, basis) => {
	div.c("basis", () => page.previews().style({ "--column": "100%", "--gap": "calc(var(--gap) * 0.35)" })).style("--basis", basis);
...
const vault = () => {
	const site = sample({
		...
		content(){
			this.$trail = div.c("flex gap v-center").style("--gap", "calc(var(--gap) * 0.4)");
			div.c("flex gap", () => {
		...
		focus(page){
			...
			this.$rail.empty(() => at.previews().style({ "--column": "100%", "--gap": "calc(var(--gap) * 0.35)" }));

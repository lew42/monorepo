...
const tile = (label, value) => div.c("pad surface flex v gap", () => {
	span.c("h2", value);
	span.c("muted", label);
}).style({ "--gap": "calc(var(--gap) * 0.1)", "--pad": "calc(var(--pad) * 0.8)" });
...
const throughput = () => div.c("flex gap flex-1", () =>
	...
	.style({ alignItems: "flex-end", "--gap": "calc(var(--gap) * 0.4)", minHeight: "4em" });
...
const feed = () => div.c("flex v gap", () => [
	...
	span(what);
	span.c("muted", when);
}))).style("--gap", "calc(var(--gap) * 0.55)");

const panel = (title, fn) => div.c("pad surface flex v gap flex-1", () => {
	p.c("h4", title);
	fn();
}).style({ "--pad": "calc(var(--pad) * 0.9)", minWidth: "0" });
...
const bays = () => div.c("grid gap auto", () => {
	for (let i = 1; i <= 48; i++)
		...
		}).style("--pad", "calc(var(--pad) * 0.6)");
}).style({ "--column": "7em", "--gap": "calc(var(--gap) * 0.6)" });
...
const depot = () => new Page({
	...
	render(){
		return this.view ??= div.c("page full fill flex", () => {
			...
			}).style({ "--basis": "9em", "--gap": "calc(var(--gap) * 0.4)", "--pad": "calc(var(--pad) * 0.9)" });

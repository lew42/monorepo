...
const cells = () => [1, 2, 3, 4, 5, 6, 7, 8].forEach(x =>
	div.c("pad wash", "cell " + x).style("--pad", "calc(var(--pad) * 0.9)"));

const wall = () => div.c("grid gap auto", cells).style({ "--gap": "var(--gap)", "--column": "14em" });
...
export default new Page({
	...
	preview(nav){ return this.preview_card(nav, () => div.c("zoom-50 pad", () =>
		wall().style({ "--column": "6em", "--gap": "calc(var(--gap) * 0.6)" }))); },

...
Nine presentational layouts at [`/layouts/labs/decks/`](/layouts/labs/decks/), each a real url you can
open cold. [`/layouts/labs/screens/`](/layouts/labs/screens/) found the two words a whole screen is made
...
```js /layouts/labs/decks/<name>/page.js
...
[`doc/regions`](/layouts/labs/decks/doc/regions/).

## The nine

| | cut | verdict |
|---|---|---|
| [Half](/layouts/labs/decks/half/) | 50 / 50 | The only cut that claims the two things are peers. Wrong for a statement + caption; right for a versus. |
| [Golden](/layouts/labs/decks/golden/) | 61.8 / 38.2 | The workhorse. At 3440 the minor share is 1313px — a capped block *inside* a region, not 1313px of prose. |
| [Aside](/layouts/labs/decks/aside/) | 70 / 30 | 30% of 3440 is 1031px. A bare nav list there is a chevron chasm; the aside has to hold something with prose in it. |
| [Triptych](/layouts/labs/decks/triptych/) | 25 / 50 / 25 | The presenter's shape — place, statement, notes. Flanks of 479px at 1920 are real rails; at 3440 they are 859 and the list caps. |
| [Poster](/layouts/labs/decks/poster/) | 20 / 60 / 20 | **The best cut for a 3440**: 687 / 2062 / 687, a 1092px block and a 142px word in the middle. |
| [Four](/layouts/labs/decks/four/) | 2 × 2 | Four peers with no sequence. Exactly one cell takes the accent or the eye has no entry. |
| [Persistent](/layouts/labs/decks/persist/) | rail + stage | The rail is the PARENT column and the slides are its children — the row already does the whole job. Two tokens, no JS. |
| [Swap](/layouts/labs/decks/swap/) | one screen | The same four slides with `full` on each. The strip under them is redrawn per slide and still reads as persistent. |
| [The pitch](/layouts/labs/decks/pitch/) | six slides | A real deck about the framework, built from the cuts above. Click, arrows and Back all move it. |
...
113px at 1920. [`doc/decisions`](/layouts/labs/decks/doc/decisions/) has the table.
...
- **A keyboard handler must die on deactivate, and only the page you are on may act** — going
  ...
  [`/layouts/labs/screens/`](/layouts/labs/screens/doc/decisions.md), reused verbatim in `deck.js`.
- **Space advances too, everywhere `arrows` is spread in** — guarded off a focused control so
  it never steals a real link's own Space behaviour: [`doc/decisions`](/layouts/labs/decks/doc/decisions/).
...
- [`doc/regions`](/layouts/labs/decks/doc/regions/) — the content-kind map: which kind belongs in a
  region of what width, with the measured widths of every cut at 400 / 1920 / 3440.
- [`doc/decisions`](/layouts/labs/decks/doc/decisions/) — the record: the head-to-head table, what

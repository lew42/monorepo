...
Judged at **3440 first**, then 1920, then 400 — the verdicts are ranked in [readme](/layouts/labs/blogx/readme/), and every measurement is in [doc/decisions](/layouts/labs/blogx/doc/decisions/).`);

		this.previews();

		md(`### What the eight are for

**Fronts** — [Magazine](/layouts/labs/blogx/front/), [Dashboard](/layouts/labs/blogx/board/), [Deck](/layouts/labs/blogx/deck/), [Columns](/layouts/labs/blogx/finder/): four answers to *what fills 3440 above the fold*.
**Rails** — [Two-level](/layouts/labs/blogx/rail/) and [Dynamic](/layouts/labs/blogx/dig/): the same archive as a fixed tree, and as a rail that changes with depth.
**Parts** — [As columns](/layouts/labs/blogx/parts/) and [In place](/layouts/labs/blogx/swap/): a four-part post read side by side, and swapped under a strip that never moves.

The one rule they all keep: **no reading column is ever over 42em.** A wide screen is filled with more columns, never a wider one — and the number that proves it is [Parts as columns](/layouts/labs/blogx/parts/) at 3440: **252 + 4 × 720 = 3132px** of live text in a 3170px row, with nothing over the measure.`);
	},
});

 ...
 *   is a post, so /layouts/labs/blogx/front/layout-generators/ cold-loads — the card
...
export class Blog extends Page {
	...
	// `http://localhost:8110/layouts/labs/blogx/<slug>/`, saved to `shots/<slug>.jpg`.
	preview(nav){
		const slug = nav.url.replace(/\/$/, "").split("/").pop();

		return this.preview_card(nav, () => img().attr("src", `/layouts/labs/blogx/shots/${slug}.jpg`).attr("alt", `The ${nav.label ?? nav.title} shell`).attr("loading", "lazy")

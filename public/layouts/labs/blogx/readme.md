...
Open [/layouts/labs/blogx/](/layouts/labs/blogx/) and click a card.

## Use

```js /layouts/labs/blogx/front/page.js
...
1. **[Magazine front](/layouts/labs/blogx/front/)** — hero + wall + two rails. The strongest
...
2. **[Two-level rail](/layouts/labs/blogx/rail/)** — sections over posts. The right rail for an
   archive under about forty posts; `active` and `in-path` are free.
3. **[Parts as columns](/layouts/labs/blogx/parts/)** — a four-part post read side by side.
   **252 + 4 × 720 = 3132px of live text at 3440**, and not one column over the measure.
4. **[Dynamic rail](/layouts/labs/blogx/dig/)** — one rail, three contents, by depth. What the
   two-level rail becomes when the archive outgrows one screen.
5. **[Parts in place](/layouts/labs/blogx/swap/)** — a persistent part strip over a region that
   swaps. The only treatment here that works unchanged at 400.
6. **[Columns](/layouts/labs/blogx/finder/)** — a front and an archive in one shell, but never
   both on screen at once.
7. **[Dashboard front](/layouts/labs/blogx/board/)** — four ways in beats one; with every region
   equally loud there is nothing to read first.
8. **[Deck front](/layouts/labs/blogx/deck/)** — the best composition and the worst overview: a
...
- **A grid can be the right height with a content-sized row.** `min-height: 100%` and
  ...
  that had already fitted. [`doc/decisions`](/layouts/labs/blogx/doc/decisions/)
...
- Four of the eight posts carry `real: "<section>/<name>"` — a link to the actual post at
  ...
  [`doc/decisions`](/layouts/labs/blogx/doc/decisions/).
...
- A `hug` column is its **max-content** width, so it hugs a list and gives prose a 24em
  note. The section columns in [`finder/`](/layouts/labs/blogx/finder/) carry no `content()`.
...
- [`doc/decisions`](/layouts/labs/blogx/doc/decisions/) — every measurement, what was rejected, and
...
- Related: [`core/Page/doc/columns`](/framework/core/Page/doc/columns/) ·
  [`/layouts/labs/shells/`](/layouts/labs/shells/) (the 3×3 chrome grid this borrows) ·
  [`/layouts/labs/screens/`](/layouts/labs/screens/) (display type sized off its own block)

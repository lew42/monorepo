...
**`/imagine/layouts/`'s `3.\*` entries and `LayoutsCard`, before that realm was deleted
2026-09-18.** That realm owned *arrangement* — every way a page divides its room, numbered,
each entry one object in `system.js`. Its `LayoutsCard` was one fixed three-column shape
(intro | stage | readouts) whose job was to *present* an entry.
...
A **screen** section (`.sections-screen`, `/layouts/labs/sections/full/`) needs a different number:
...
1. ~~A "Sections" entry on the layouts hub~~ — moot: the hub this meant
   (`/imagine/layouts/`, whose `system.js` catalogue numbered arrangements) was deleted
   2026-09-18. This realm now lives directly under [`/layouts/`](/layouts/), at
   [`/layouts/labs/sections/`](/layouts/labs/sections/).
...
- **A `space` chip on the hub's spacing section.** The three levels are shown side by side
  instead, which needs no control; the live chip exists on `/layouts/labs/sections/stack/`, where a

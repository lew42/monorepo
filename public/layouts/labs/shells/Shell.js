 ...
 *
 * Design record: /layouts/labs/shells/readme.md.
 */
...
Ship notes for the build every shell in this lab is wrapped around. This document does not change from page to page — the chrome around it does.`);
	...
	// `http://localhost:8110/layouts/labs/shells/<slug>/`, saved to `shots/<slug>.jpg`.
	preview(nav){
		const slug = nav.url.replace(/\/$/, "").split("/").pop();

		return this.preview_card(nav, () => img().attr("src", `/layouts/labs/shells/shots/${slug}.jpg`).attr("alt", `The ${this.title} shell`).attr("loading", "lazy")

...
sidebars/footers, etc"*. Ten shells at `/layouts/labs/shells/`, each its own screen, each with a
...
`/layouts/labs/screens/` reached independently the same night (695→1439 at 1280, 856→1747 at 3440
...
make `/layouts/labs/shells/` depend on `/imagine/paging/`, a cross-realm import neither module
declares today) and is left as the open question below.

**Persistence: none.** `grep -rn "store()\|localStorage" public/layouts/labs/shells/` returns

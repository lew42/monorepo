	...
	children: [
		{
			...
			content(){ md("**The row divides itself; the height does not.** A hop to the right is a column and costs nothing but a url. A hop downwards has no row to open into, so the screen redraws with one more band — see [Stack](/layouts/labs/screens/stack/). Both are the same page tree; only one of them is free."); },
		},
		{
			title: "Count", width: "fill", classes: "screens-column",
			content(){ md("**Four is where it stops being useful.** At 1920 a quarter of the row is 480px, which is a rail with a headline in it. Past four the row scrolls instead of shrinking, and scrolling a row of screens is a different experience than dividing one — see [Divide](/layouts/labs/screens/divide/)."); },
		},
		{
			title: "Ratio", width: "fill", classes: "screens-column",
			content(){ md("**A basis is a share.** Columns asking for 61.8% and 38.2% of a row are shrunk in proportion, so those two numbers are the golden section at every width — no grow weights, no media queries. [Uneven](/layouts/labs/screens/uneven/) is three of them."); },
		},
		{
			title: "Swap", width: "fill", classes: "screens-column",
			content(){ md("**Two words, and that is the whole space.** A page saying `full` replaces the screen you were on; one saying `fill` joins it and everyone shares. A deck is the first; a split is the second; a cover is the first that changed its mind — [Title Slide](/layouts/labs/screens/title/)."); },
		},
	],
});

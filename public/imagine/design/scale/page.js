import { Page, div, span, h2, h3, p, a, img, figure, figcaption, md } from "/app.js";
...
}).style({ gap: "calc(var(--gap) * 0.3)", borderBlockEnd: "1px solid var(--line)", paddingBlockEnd: "var(--pad)" });

const SPECTRUM = {
	390:  { distinct: 18, rows: [[11, 35.8], [13, 24.5], [12, 18.0], [14, 15.8], [10, 3.6]] },
	1280: { distinct: 19, rows: [[12, 36.3], [14, 25.2], [15, 18.3], [13, 14.2], [11, 2.3]] },
	3440: { distinct: 22, rows: [[15, 32.4], [16, 21.1], [18, 18.3], [14, 11.8], [17, 10.7]] },
};

const bar = ([px, pct]) => div.c("flex v-center gap", () => {
	span(px + "px").style({ width: "3.5em", flex: "0 0 auto" });
...
const shot = s => figure.c("flex v gap", () => {
	...
	figcaption(() => {
		span(s.verdict + " — ").style({ fontWeight: "700", color: s.verdict === "GOOD" ? "var(--prim)" : "inherit" });
		a(s.url).href(s.url);

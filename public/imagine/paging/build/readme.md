...
- [`../doc/builder.md`](/imagine/paging/doc/builder/) — the census of all 890 `page.js` on the
  site, the ruling that navigation is one control, the tabs answer, and the four steps that turn
  this stage into the realm's own `PagingStage`.
...
- Files: `page.js` (the page and its seven controls) · `words.js` (the vocabulary and the node
  operations, imports nothing) · `stage.js` (`BuildStage` — a node, drawn; [it should be `PagingStage`](/imagine/paging/doc/builder/)) · `build.css`

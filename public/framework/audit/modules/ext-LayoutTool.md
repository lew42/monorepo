...
7. **Outside-the-box: let a rule optionally publish its fix as a named custom
   ...
   `{ prop: "--lt-fix-measure", value: "52em" }`. The readme's own Open
   ...
   CSS ever come to define `--lt-fix-measure`?) instead of purely a human's
...
`score.js` → `LayoutTool.js`, plus `report.js`, `live.js`, `mirror.js`) is
...
**What moving it would cost**, if the call went the other way (`ext/LayoutTool`
→ `dev/LayoutTool`, or splitting the pure core from the dev-only shell into
two directories): every absolute `/framework/ext/LayoutTool/...` reference
...
in the half-dozen files elsewhere on the site that mention `ext/LayoutTool` in
...
Not the Editor/Panel/`ext/layout`/DevBar/demo cluster — `LayoutTool` is
...
- **`styles/layouts/space`** is the closest structural cousin: `ruler.js`
  ...
  `data-layout-ignore` for LayoutTool's benefit. Both modules exist to make a
  layout's behavior across widths *inspectable*; `space` renders the
  comparison, `LayoutTool` scores it. Not a merge candidate — one is a demo
...
- **The `layout-design` skill** is this tool's human-facing counterpart by
  ...
  skill is the authored *how to build* guidance, `LayoutTool` is the
...
- **No guidance for a module that's genuinely "many files, each its own
  ...
  exports of `LayoutTool.js` and left `report`/`mirror`/`defer`/`vision`/

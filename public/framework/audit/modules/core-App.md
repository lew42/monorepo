...
| callers | 3 real, live imports — `/app.js` (production boot, the only one), [`core/Sidebar/Sidebar.js`](/framework/core/Sidebar/) (imports `mode` for the footer toggle), `framework/start/example/app.js` (teaching copy). Everything else that mentions `App.js` — `core/page.js`, `core/Page/shell/page.js`, sandboxes, other modules' docs — quotes `new App()` inside a code example, not a live import. `core/new/0`, `core/new/1`, `core/new/starter` each vendor their **own** `App.js`. |

...
| callers | **Socket**: 5 — `public/app.js` (boots it), `ext/Ask/Ask.js`, `ext/Saver/FileSaver.js`, `ext/DesignTool/audit/twin.js`, `dev/DevBar/tools.js` (reads status only). **DevBar**: 1 — `public/app.js` (`render()`/`navigated()`), which is also its entire public surface by design. |

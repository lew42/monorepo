...
`demo()` is the first caller and would have been a fine home. The stated reason for
moving it was the same one `source()` set: **a second caller was already obvious** —
an element reference wanting "show me this element's markup," so that a page
reimplementing the same idea wouldn't print the same `div` two ways.

**That second caller has not shown up.** A caller grep today (2026-08-15) finds
exactly one: `ext/demo/demo.js:94`. The move to `util/` is still the right call —
`source()` proved the pattern, and one real caller plus an obvious second one is a
reasonable bar — but the second one is anticipated, not real, and this file should
say so rather than imply it already happened. If nothing else calls `markup()` by
the time this is read again, that's worth re-litigating.
...
- **The second caller `util/`'s move was justified by still doesn't exist.** See §2.
  Not a reason to move it back — `source()` earned the same move on the same
  reasoning and paid off — but worth re-checking, not just re-asserting.

# wire-c — redraw 9 websites content-free, from the screenshots already on disk (Sonnet, group `websites`)
...
## Your sites — these 9 and no others

| name | title | layout at 1920 | at 400 | breakpoints |
|---|---|---|---|---|
| `amazon-kindle-product` | Amazon.com: Apple 2024 MacBook Air 13-inch Laptop  | 3-holy-grail | 2-sidebar | 768,979 |
| `cctype-foundry` | CcTimeline – Contemporary Font Family by CcType |  | 1-flow | 1-flow | 768,900,1280 |
| `figma-home` | Figma: The collaborative canvas for design, code,  | 1-bands | 1-bands | 400,768,1024 |
| `grafana-play-home` | Dashboards - Grafana | 2-sidebar | 1-flow | 544,769,992,1200 |
| `kubernetes-docs-home` | Kubernetes Documentation | Kubernetes | 3-holy-grail | 1-flow | 768,1024,1500 |
| `nike-home` | Nike. Just Do It. Nike.com | 4-1 | 4-1 | 1024 |
| `overreacted-blog` | overreacted — A blog by Dan Abramov | 1-centered | 1-flow | 672 |
| `shopify-polaris-home` | Polaris references | 2-sidebar | 1-flow | 640,960,1920 |
| `svelte-docs-overview` | Overview • Svelte Docs | 3-holy-grail | 1-flow | 480,768,900 |
...
3. **Write it through the merge** — a scratchpad script `wire-c-write.mjs` that reads the record with `read_record`, sets `wire`, writes with `write_record`. Never rewrite another key; never hand-edit the json.
4. **Check the drawing**: load `http://localhost:8123/websites/<name>/` headless at 1920×1080 (a scratchpad probe `wire-c-check.mjs`; the Playwright import is in the rules), screenshot the page, Read it, and compare the three wires against the real shot above them. Adjust once if a stranger would not match them; then move on. Write one honest word per site in your log: `close`, `rough`, or `wrong` (with why).

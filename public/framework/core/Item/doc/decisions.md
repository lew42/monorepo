...
[List](/framework/core/List/), an id, and a saver somewhere up the chain. No view, no
...
deliberately excluded from it: [envelope.md](envelope.md).
...
- `Item.js` carries the ruling's one unavoidable import (`../List/List.js`).
  Still headless and node-runnable; see `/framework/core/List/readme.md`.

...
option was rejected: the type already says it. `./method/append.md`.
...
`<dvi>` element. `./method/elements.md`.
...
`./method/stylesheet.md`.

**Why does `style()` special-case `--x`?** `el.style["--x"] = v` silently does
nothing — custom properties only respond to `setProperty`. `./method/style.md`.
...
is memory that must be invalidated. `./method/on.md`.

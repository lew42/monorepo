...
one (`App.js:63`), which is why this has never bitten.

## Simplicity

Right-sized at six lines. Two subjects — a click and a history pop — but they are
one moment (*"start listening"*), and splitting them gives two names for it.

One nit: the constructor logs `"router.listen() — click + popstate wired"`
(`Router.js:6`) about work this method did. The line belongs here, or nowhere.

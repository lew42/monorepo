# Page overview demos — fifteen live `Page` trees, one directory each, under `/framework/core/Page/old/overview/`

## Use
```js /framework/core/Page/old/overview/<name>/page.js
...
```
Add the name to `children:` in `old/overview/page.js` — the string is the tab order; `group:` no longer draws anything (the 4th-level experiment is flat top tabs, not a grouped rail).

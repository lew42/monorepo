...
export default new Page({
	...
	children: [
		demo.page("list", list, {
			note: "A tag *list* needs no field, so it needs no opt-out at all — `flex wrap gap` and four [badges](/framework/ui/badge/). This is the honest first answer, and the field is for when they are editable." }),

		demo.page("plain", plain, {
			note: "And the version that fights nothing: a [form field](/framework/ui/field/) whose value happens to be a comma-separated list. Two lines, no chips, worth wanting before the pretty one." }),
	],
	...
	content(){
		...
		demo.exhibit({
			page: this,
			stage: steer => demo.stage(tags, steer).ac("bleed"),
			def: tags,
	...
	preview(nav){ return this.preview_card(nav, () => div.c("zoom-50 pad", tags)); },
});

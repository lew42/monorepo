...
const FLAWS = [
	...
	},
	{
		...
		ev: `Of eleven bands only \`contrast\` touches type size, and it is saturated. Meanwhile [scale study](/framework/styles/system/studies/scale/): the commonest font-size on the site is *smaller than body* at 390, 1280 and 3440. And \`audit/findings.json\` and \`audit/taste.json\` are both stamped **2026-08-17**, 338 rows, unreran; \`learned.md\`'s own step 1 ("Mike ranks the 18 shots") is still open, so every tier is unvalidated.`,
...
const EVIDENCE = [
	...
	"|---|---|---|",
	"| [layout](/layouts/doc/studies/) | **3 shells** cover 20/20 pages; **0** pages overflowed at any width | the system handles three shapes and one direction of failure |",
	"| [layout](/layouts/doc/studies/) | `/imagine/gallery/` **13.2%** of 3440 — at 1280 *and* 3440 | a column with no way to say \"grow with the row\" → `holds: many` + a `grow` width word |",
	"| [layout](/layouts/doc/studies/) | rail+content **45% → 19%** (`/michael/`) from 1280 to 3440 | a prose ceiling with no declared second region → `holds: parts` |",
	"| [scale](/framework/styles/system/studies/scale/) | the **modal font-size is smaller than body** at 390, 1280 and 3440 | chrome outnumbers prose; content type gets a floor of 1em and a container clamp |",
	"| [scale](/framework/styles/system/studies/scale/) | **one** element site-wide scales with the viewport — `clamp(2.2rem, 17cqw, 12rem)` | the mechanism already exists and is used once; `holds: one` makes it the default |",
	"| [padding](/framework/styles/system/studies/padding/) | comfortable band **0.5em → min(6% width, 3em)** | the padding floor, verbatim |",
	"| [padding](/framework/styles/system/studies/padding/) | **0 of 2178** real boxes exceeded 20% of their width | the ceiling never binds — the floor is the whole game |",
...
export default new Page({
	...
	content(){
		prose("**A proposal, not a change.** Nothing in `framework/core/` or [`ext/DesignTool/`](/framework/ext/DesignTool/) was touched tonight. This is the design the owner asked for, written against the tool as it stands and tonight's three studies — [layout](/layouts/doc/studies/), [scale](/framework/styles/system/studies/scale/), [padding](/framework/styles/system/studies/padding/) — which agree on one thing: **every failure on this site is dead space or cramped. Not one page overflowed.**");

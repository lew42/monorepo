...
// The shots stay where they were, beside the study this page detached from.
const here = new URL("../", import.meta.url).pathname;

/**
 * Moved here 2026-09-17: this was `/framework/styles/system/studies/color/` itself until the lighten/darken
 * study took level 1. Nothing was cut — it is one click down, where the detail belongs.
 *
...
const swatch = ([name, paints, lhex, dhex]) => figure.c("flex v gap").style({ margin: 0, gap: "calc(var(--gap) * 0.35)" }).append(() => {
...
const FAILS = [
	...
	{ file: "fail-stray-blue-link.jpg", ratio: "8.4:1 (not a contrast defect — a palette defect)", where: "/layouts/doc/studies/ — bare <a> in body content",
...
const OKS = [
	...
	{ file: "ok-padding-clean.jpg", where: "/framework/styles/system/studies/padding/, light", what: "One page, one job: cards on --wash, ink text, a single underlined link. Nothing competes with the accent because nothing else is trying to be one." },
...
export default new Page({
	meta: import.meta,
	title: "Palette",
	description: "Every color token this theme defines, what each paints, measured usage, and the seven real contrast failures.",
	icon: "palette",
	width: "full",

	// A real screenshot rather than the default icon+description card (2026-09-05 ux-rethink).
	preview(nav){
	...
	content(){
		...
		div.c("color-pairbox").style({ gridTemplateColumns: "repeat(auto-fill, minmax(min(22em, 100%), 1fr))" }).append(() => FAILS.map(f => figure.c("flex v gap color-fail").style({ margin: 0, gap: "calc(var(--gap) * 0.4)" }).append(() => {
			shot(f.file, f.where);
			figcaption(() => {
		...
		div.c("color-pairbox").style({ gridTemplateColumns: "repeat(auto-fill, minmax(min(22em, 100%), 1fr))" }).append(() => OKS.map(o => figure.c("flex v gap color-ok").style({ margin: 0, gap: "calc(var(--gap) * 0.4)" }).append(() => {
			shot(o.file, o.where);

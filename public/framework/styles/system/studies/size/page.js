...
/* THE FIVE QUESTIONS (layout skill)
   1. Container — a plain page under `styles/system/studies/`, like every sibling study
      (moved from `/imagine/design/` 2026-09-18, `ai/2026-09-18/imagine-move-2/`). The
      ordinary page grid applies here now, not a columns host's bare column.
...
export default new Page({
	...
	/* A real screenshot instead of the icon+title card, matching every sibling study
	   (the wall is a wall of pictures). `.design-shot` — now core's own rule, in
	   `core/Page/Page.css` beside `.page-preview-thumb` (moved there 2026-09-18, it
	   used to be this realm's own `design.css`) — fills and crops the card's thumb
	   slot, which a bare `<img>` does not. */
	preview(nav){

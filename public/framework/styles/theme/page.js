...
export default new Page({
	...
	content(){
		...
		}, "`pre` is a **block** and `code` is **inline**, so they get different padding — 0.75em/1em against 0.15em/0.4em. One shared value fit neither, which is how `md.css`, `demo.css` and `syntax.css` each ended up overriding it independently. `pre > code` resets to zero, since the block is already a padded box.");

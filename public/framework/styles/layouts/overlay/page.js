...
export default new Page(demo.layout({
	...
	layout(){
		...
		const row = (title, note, on) =>
			...
			}).ac(on && "wash").style({ "--pad": "calc(var(--pad) * 0.55) 0", borderRadius: "var(--radius)" });
		...
		return div.c("page full fill flex v", () => {
			...
			if (this.shows("sheet"))
				div.c("surface measure pad flex v gap", () => {
					...
					div.c("flex v gap", () => OPTIONS.forEach(o => row(...o))).style("--gap", "calc(var(--gap) * 0.3)");

				}).style({ position: "absolute", insetInline: "0", insetBlockEnd: "0", "--gap": "var(--gap)" });
			...
			if (this.shows("menu"))
				div.c("surface pad flex v gap", () => {
					...
					div.c("flex v gap", () => site.topics.split(" ").forEach((word, i) =>
						row(word, "", i === 0))).style("--gap", "calc(var(--gap) * 0.2)");

				}).style({
					position: "absolute", insetBlock: "0", insetInlineStart: "0",
					width: "min(20em, 100%)", "--gap": "calc(var(--gap) * 1.2)",
				});

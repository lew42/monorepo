# Layout space — decisions and record
...
words/      every word the format accepts, each one live. syntax.md is the grammar
```
...
utilities is a real proposal and it is the owner's call, not this page's; until then
...
**Verdict (the owner, 2026-08-16): controls above, five in one row, one scale.** Each
...
## A grammar, not a flat pick (2026-08-16, the owner)
...
category errors rather than bad arrangements. The owner's phrasing: *we don't
...
## Chaos: one dial, and it is a distance (2026-08-16, the owner)
...
## The tones are the site's, not invented (2026-08-16, the owner)
...
1. **A rulebook with two sides.** `ext/DesignTool/taste/ranges.js` holds eleven
...
### Decision: a second turn of the crank plateaus (2026-08-17)

**Does the loop compound? No — it plateaus, and the held-out check says it is a
real plateau, not overfitting.** Full method and raw numbers:
`ai/2026-08-17/generator-compound/`.

Round 3 (baseline, the `SHAPES` this file already carried) vs round 4 (those
weights refit off round 3's own data, `credit()` → `proposal()`, applied),
same population both times — pooled `sweep()` over depth 1/2/3, chaos 0, seeds
0–119, n=360:
...
+0.40 of a point on a ±8-point-std population is noise, not lift — under one
standard error of the difference. Ceiling and the 10th percentile (the floor
that matters more than the ceiling for a generator someone will actually
click) **did not move at all.**

**Held-out (seeds 10000–10119, never touched by either fit) says the same
thing, not a worse thing** — which is what rules out overfitting rather than
just failing to find lift: mean +0.075 (0.15 SE), ceiling +0, p10 +0. The
fit-range gain is not bigger than the held-out gain; both are indistinguishable
from zero. A memorising loop looks like fit-range up, held-out flat-or-down —
this is flat-or-flat.

`model.js` was left at round 4's weights (nothing regressed, every metric is
nominally ≥ round 3) but the honest framing is **tied within noise**, not
improved. Refitting a third time off this same 360-seed population is unlikely
to find anything either — the two ingredients that would change that are a
bigger `n` or a different signal than worst-width-by-shape, not another pass.

### Decision: where the 33 points actually go (2026-08-17)

**`SHAPES` reweighting was the wrong knob — it can only shift the mix of rolls,
never the quality of any one roll.** Before turning it a third time, the same
FIT population (depth 1/2/3, chaos 0, seeds 0–119, n=360) was re-swept keeping
`rate()`'s per-band breakdown for the worst-of-3-widths mark of every row (not
just the aggregate score) — the mark fitness is actually judged on. Method and
the full ranked table: `ai/2026-08-17/loss-budget/`.

Four of the eleven `taste/ranges.js` bands account for **84% of the 33-point
gap**; the other seven — including `slivers` and `depth`, both at exactly
0.00 — split the remaining 16%, confirming and sharpening this file's own
"five ranges carry no information here" finding above.

| band | share of gap | mean credit | why |
|---|---|---|---|
| `width-used` | 28.1% | 0.360 (0.062 when 3440 is the worst width) | fixable — see below |
| `repetition` | 21.0% | 0.012 | structural |
| `measure` | 17.9% | 0.600 | tangled with `width-used`, below |
| `contrast` | 17.1% | 0.375 (0.655 on `bands` only) | structural |

**`repetition` and `contrast` (38% of the gap) are not reachable by any range
tweak.** `repetition` is the direct, unavoidable cost of a part vocabulary
built from `COUNTS` runs of identically-classed children — a wall of cards or
a nav list *is* a repetition group, by the same measurement this file already
recorded for the hand-written presets. `contrast` is flat because `bands` is
the only `SHAPES` entry whose tracks ever include `feature`/`hero`, and no
`AUTHOR` range sizes heading text at all — there is no emission to move.

**`width-used`, the single largest band, IS an emission, and it is entangled
with `measure` through one line.** `gen.js`'s `claim()` applies the
`flow measure start --measure:Xem` cap to any LEAF whose drawn part is
`sections`/`notes` — not only genuine `prose`-role tracks — so 46% of `main`,
18% of `wall` and 14% of `index` leaves collapse to a centred ~30em column
inside a row that may be 3440px wide, regardless of how much room the row
actually has. That rule is the documented fix for a 117-characters-a-line bug
(above, "A grammar, not a flat pick"), so narrowing it back is a real trade
against `measure`, not a free win — which is exactly why it's an experiment
and not a rewrite.

**Net reading: about 38% of the gap needs new shapes or parts and is out of
this generator's reach as it stands; about 28–46% traces to one named
emission decision and is worth one targeted, reversible change; the rest is
spread thin across bands already averaging ≥0.75 credit — near this
generator's structural ceiling on those.** `SHAPES` was correctly left alone
a second time — the table it would have fit was never where the points were.
...
- **A long line is the most common complaint at 3440**, at zero credit on five of
  the nine — but ⚠ **it is not the token's fault, and this readme said it was.**
  A hand count settled it (`ai/2026-08-16/mastermind-layout/measure-verdict.md`):
  `--measure: 52em` is **~100 characters** (106 on a full line of plain prose,
  84–108 across real copy, ~2.0 characters per em). The 113–117 readings came
  from `docs`, `shell`, `split` and `mail` — four presets that **declare no
  measure at all** and run prose on 51–59em `fluid` tracks. `document`, the only
  preset that sets the token, reads 98. So the finding is about tracks with no
  ceiling, not about the ceiling's value. `AUTHOR.measure`'s 27–34em still lands
  at 51–69 characters and is unchanged.
  ⚠ And the causal story this readme told — that the number grows with the
  viewport because the em measure rides the font-size clamp — is **false**.
  Characters per *em* is a font metric; the count is bit-identical at 1280, 1920
  and 3440.
...
⚠ **`repetition` reads ~0.95 on everything in this rail, and it is STRUCTURAL —
no amount of rewriting the copy moves it.** This paragraph used to say the
opposite, and a worker sent to fix it came back with the refutation instead:
`repetition` groups siblings by `probe.js`'s `label()` — tag plus its first three
classes — and **never reads the text at all**. Six `div.flex.v.gap` boxes are a
repetition whether they hold six different paragraphs or one paragraph six times,
proved by building six of each and measuring 1.00 both ways.

So the fictional site reads 0.95 because `sections`, `rows`, `notes`, `cards` and
`tiles` each emit N identically-classed children, and real pages measure 0.23
because most of their text is unique prose in a `.flow`. Moving it would mean
changing *which elements share a class*, which is a change to the parts rather
than to their words. **The copy got rewritten anyway** — the demo site now has
real topic-matched prose and a `notes` wall that finally reaches its designed 5×
ragged ratio (`ai/2026-08-16/web-prose-variety/`) — with the band, `measure`, and
every `analyze()` score bit-for-bit unchanged across 46 runs. Worth having; not
...
`fit` is the other half of the owner's question — *for a fixed size area, should it
...
## Depth replaced the detail dial (2026-08-16, the owner)
...
**The owner asked whether this generates every permutation, and whether many would look
...
**Verdict: ship it raw** (the owner). No score, no guard rails — broken rolls are the
honest output of a search, and the wall is where you see what the space actually is.
`ext/DesignTool` scoring stays phase 2, where it already was.
...
## Blocks are translucent, and the stacking is the point (2026-08-16, the owner)
...
## `compose/` — the same roll as real panels (2026-08-16, the owner)
...
- **A fixed `--basis` container collapses its children.** `share()` translates
  ...
  per line, `ext/DesignTool` **F 8**. Emitting a fixed claim only on leaves took the
...
the same layout, it rolls a different one. That is the trade the owner asked for, and it
...
`ext/DesignTool` on the Overview: **C 76 at 1280, 1920 and 3440, zero high
...
`ext/DesignTool` at 1280 / 1920 / 3440, after marking the miniatures: **C 79, C 71,
...
- **Score.** `ext/DesignTool` already grades a rendered box. Running it over each

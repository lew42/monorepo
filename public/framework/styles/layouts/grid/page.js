...
const cells = () => [1, 2, 3, 4, 5, 6].forEach(x => div.c("pad wash", "cell " + x).style("--pad", "calc(var(--pad) * 0.5)"));
		...
		demo(() => {
			div.c("flex v gap", () => {
				div.c("grid gap").style({ gridTemplateColumns: "1fr 200px" }).append(() => {
					div.c("pad wash", "1fr").style("--pad", "calc(var(--pad) * 0.5)");
					div.c("pad wash", "200px").style("--pad", "calc(var(--pad) * 0.5)");
				});
...
repeat(auto-fill, minmax(min(var(--column), 100%), 1fr))   /* fewer cells → gaps */`);
		...
		demo(() => {
			div.c("grid gap auto").style("--column", "9em").append(() => {
				div.c("pad wash", "span 2").style({ "--pad": "calc(var(--pad) * 0.5)", gridColumn: "span 2" });
				div.c("pad wash", "two").style("--pad", "calc(var(--pad) * 0.5)");
		...
		demo(() => {
			div.c("grid gap three").style("--column", "4em").append(() => {
				div.c("pad wash", "hero").style({ "--pad": "calc(var(--pad) * 0.5)", gridColumn: "span 2" });
				div.c("pad wash", "b").style("--pad", "calc(var(--pad) * 0.5)");
			});
		...
		demo(() => {
			div.c("grid gap").style({ gridTemplateColumns: "1fr 1fr", gridTemplateAreas: '"a a" "b c"' }).append(() => {
				div.c("pad wash", "a").style({ "--pad": "calc(var(--pad) * 0.5)", gridArea: "a" });
				div.c("pad wash", "b").style({ "--pad": "calc(var(--pad) * 0.5)", gridArea: "b" });
				div.c("pad wash", "c").style({ "--pad": "calc(var(--pad) * 0.5)", gridArea: "c" });
			});
		...
		demo(() => {
			div.c("flex v gap", () => {
				div.c("grid gap").style({ gridTemplateColumns: "1fr 1fr" }).append(() => {
					div.c("pad wash", "pneumonoultramicroscopicsilicovolcanoconiosis").style("--pad", "calc(var(--pad) * 0.5)");
					div.c("pad wash", "normal cell").style("--pad", "calc(var(--pad) * 0.5)");
				});
		...
		demo(() => {
			div.c("flex v gap", () => {
				div.c("grid gap auto").style("--column", "10em").append(() => {
					div.c("pad wash", "1").style("--pad", "calc(var(--pad) * 0.5)");
					div.c("pad wash", "big").style({ "--pad": "calc(var(--pad) * 0.5)", gridColumn: "span 2" });
					div.c("pad wash", "2").style("--pad", "calc(var(--pad) * 0.5)");
					div.c("pad wash", "3").style("--pad", "calc(var(--pad) * 0.5)");
				});
		...
		demo(() => {
			div.c("grid gap auto").style("--column", "8em").append(() => {
				div.c("pad wash", "start").style({ "--pad": "calc(var(--pad) * 0.5)", justifySelf: "start" });
				div.c("pad wash", "center").style({ "--pad": "calc(var(--pad) * 0.5)", justifySelf: "center" });
				div.c("pad wash", "end").style({ "--pad": "calc(var(--pad) * 0.5)", justifySelf: "end" });
			});
		...
		demo(() => {
			div.c("grid gap auto").style("--column", "10em").append(() => {
				...
				div.c("pad wash grid gap auto", () => {
					div.c("pad wash", "2a").style("--pad", "calc(var(--pad) * 0.3)");
					div.c("pad wash", "2b").style("--pad", "calc(var(--pad) * 0.3)");
				}).style({ "--pad": "calc(var(--pad) * 0.5)", "--column": "4em" });
				div.c("pad wash", "3").style("--pad", "calc(var(--pad) * 0.5)");
			});

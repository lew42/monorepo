...
).style({ background: "var(--surface)", padding: "calc(var(--pad) * 2.5) calc(var(--pad) * 2)" });
...
const card_list = () => div.c("flex v gap surface pad", () =>
	[
		...
		a.c("pad flex gap v-center wash").href("#").style({
			...
			div.c("flex v").style("gap", "calc(var(--gap) * 0.15)").append(() => {
				p.c("h4", title);
				p.c("muted", desc);
			});
...
const bottom_sheet = () => div.c("flex v").style({
	...
	div.c("pad flex v gap surface").style({
		...
		borderRadius: "1em 1em 0 0", "--gap": "calc(var(--gap) * 0.5)",
	}).append(() => {
...
const spec_row = (row, shows) => div.c("flex gap wrap", () => {
	...
	div.c("flex v gap").style({ flex: "999 1 18em", minWidth: "0" }).append(() => {
		...
		if (row.badges) div.c("flex gap wrap", () => row.badges.forEach(chip)).style("--gap", "calc(var(--gap) * 0.4)");
		...
		return div.c("page flow surface", () => {
			...
			if (this.shows("header"))
				div.c("wide flex gap wrap split v-center", () => {
					...
					div.c("flex v gap").style({ "--gap": "calc(var(--gap) * 0.3)", flex: "1 1 16em", minWidth: "0" }).append(() => {
			...
			TIERS.forEach(tier =>
				div.c("wide wash pad flex v gap").style({ "--gap": "var(--flow)", "--pad": "calc(var(--pad) * 1.5)", borderRadius: "var(--radius)" }).append(() => {

					div.c("flex v gap").style("--gap", "calc(var(--gap) * 0.3)").append(() => {
						h2(tier.title);

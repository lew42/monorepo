...
entry.js    spec → a twin-card layout page, wired for /full/ (ext/LayoutTool's seam)
...
[`ext/LayoutTool/widths/`](/framework/ext/LayoutTool/widths/) is the live meter that
...
- ⚠ **The exact `route()` full.js's own docstring and `direction.md` quote —
  ...
  `/full/` url just renders empty (`ext/LayoutTool` reported "5 characters of

...
export default new Page(demo.layout({
	...
	layout(){
		...
		const band = (tone, fn) => div.c("pad", () =>
			div.c("measure flex v gap", fn).style({ "--measure": "62em", "--gap": "calc(var(--gap) * 1.2)" }))
			.ac(tone).style("--pad", "calc(var(--pad) * 3.5) calc(var(--pad) * 2)");
			...
			div.c("flex-1", () => {
				...
				if (this.shows("cta")) band("", () => {
					div.c("flex gap wrap v-center split", () => {
						div.c("flex v gap", () => { h2("Start today"); p(site.blurb); }).style("--gap", "calc(var(--gap) * 0.4)");

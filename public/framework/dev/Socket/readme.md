...
One WebSocket to the dev server, and the transport every dev-only feature rides.
It reloads the document when a file it loaded changes, hot-swaps a stylesheet in
place when it can, announces which page this tab is on, and carries whatever else
asks — file writes, the CLI bridge, streamed log lines, an expression to evaluate.

```js
new App({ socket: Socket.singleton() });   // public/app.js
```
...
page updates in place. [wire](/framework/dev/Socket/docs/wire/) is the protocol.
...
| `ext/LayoutTool/audit/twin.js` | queues an accepted layout fix to disk via `async_rpc("write", …)` | [LayoutTool audit](/framework/ext/LayoutTool/audit/) |
...
(`ext/Saver/FileSaver.js`) and `LayoutTool/audit/twin.js`'s `accept()`, both
...
for them. Full accounting: [wire](/framework/dev/Socket/docs/wire/).

...
A tab, claimed by an agent: a ring around the viewport, a label at the top
centre, and a 🟠 in the title. Two functions and one stylesheet.

```js
import { claim, release } from "/framework/dev/Claim/claim.js";

claim("claude", "layout-generator-rules");   // → "claimed /framework/… — claude"
release();
```

...
import mini_app from "../core/Page/overview/demos/mini-app.js";
...
export default new Page({
	meta: import.meta,
	title: "Report",
	description: "2026-08-08 — a page names itself, a class page is three tabs, and every core member has a url.",
	icon: "summarize",

	// the demos want the breakout tracks; the prose stays a reading column
	classes: "grid",
	...
	content(){
		...
		mini_app(tour(), { nav: true }).ac("wide").style("height", "20em");

		md("A real `Page` tree in a box: rail, cards, crumbs and region, every one of them derived from `nav_for()` and `chain()`. `mini_app()` plays App and Router for that one tree, so the url in your address bar never moves. **`nav:` maps are gone** — a label, an icon and a card size are declared on the page they name, and the last `nav:` on the site was in the page you are reading. [Seven more](/framework/core/Page/overview/demos/).");

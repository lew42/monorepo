...
| `thread()` | `Ask.js` — open a thread dir; no process. See [task](docs/task/) |
| `start()` | `Ask.js` — a NEW task, not a turn; see [task](docs/task/) |
...
| [`ext/LayoutTool/vision.js`](/framework/ext/LayoutTool/) | `ask()` with `shot`, locked to `Read,Glob,Grep` — a second opinion on a numeric layout report | [`/framework/ext/LayoutTool/`](/framework/ext/LayoutTool/), its [`audit`](/framework/ext/LayoutTool/audit/) |
...
and why `thread()` no-ops on an existing dir: [task](docs/task/).
...
Long version, including what "Phase 2" would cost: [process](docs/process/).
...
blind. Full mechanism and a measured cost: [shot](docs/shot/).
...
file, no database. Detail: [record](docs/record/).
...
- **⚠ The prompt is browser input reaching a process spawn.** `spawn` runs with
  ...
  The `task` path is fenced by `thread_dir()`; see [task](docs/task/).

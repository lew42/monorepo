# highlight — decisions and record
...
Long form: `./choice.md` (which highlighter, and why five languages),
`./hooks.md` (block-awareness, the two hook points, and the FOUC
question), `./chaining.md` (the one sharp edge, and every workaround).
...
[`file-labels`](/framework/ext/markdown/doc/file-labels/) note for that
...
chaining sharp edge, is `chaining.md`.
...
**synchronous**, which is worth more than it looks. `./choice.md`.
...
(`method/lang.md`); the fix belongs to `ext/Doc`/`core/View`, not here.

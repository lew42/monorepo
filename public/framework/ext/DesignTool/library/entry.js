 ...
 * page's own width, a live census that follows the window, and the same pattern
...
export function entry(spec){
	return {
		...
		/* ⚠ The page is `dt-page` — full width, no measure — because the SPECIMEN
		...
		content(){
			...
			const $body = div.c("dt-case-body").append(spec.build);
...
export async function measured(url, width, root = ".dt-case-body"){
	try { return { report: await frame(url, width, { root }) }; }
	catch (error){ return { error: error.message }; }
}

export function widths_table(rows){
	return ui.table(["Width", "Findings", "measure", "used", "Leading finding"],

...
const url_of = title => `/framework/ext/DesignTool/library/bad/${Page.slug(title)}/`;
...
export default new Page({
	...
	content(){
		...
		md("**Fires** says whether the rule the entry was built to trip actually did, at that width. A quiet "
			...
			+ "spot and the entry says so — [Scroller in a wrapping row](/framework/ext/DesignTool/library/bad/scroller-in-a-wrapping-row/) "
			+ "is the standing example.\n\n"
			+ "The counterpart wall: [Layout library](/framework/ext/DesignTool/library/). The analyzer's own "
			+ "ground truth, which is a different question: [Test corpus](/framework/ext/DesignTool/tests/).");
	},
	...
	results(rows, width){
		...
		ui.table(["Don't", "Trips", "Fires", "Findings", "Leading finding"],
			rows.map(row => [
				() => a(row.spec.title).attr("href", url_of(row.spec.title)),
				row.spec.rule,
				row.report ? (fires(row) ? "✓" : "quiet") : "—",
				row.report ? census(row.report.counts) : "—",

"These cases of could-be-off could be deferred to the user" (the owner). A judgement
...
`localStorage`, and `analyze()` (in `DesignTool.js`) subtracts those findings
...
`localStorage` filter applied in `split()`, which is why `DesignTool.js`

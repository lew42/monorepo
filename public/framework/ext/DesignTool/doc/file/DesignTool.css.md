...
## `.dt-twin` is a pair, declared — not `auto-fit`
...
## `.page.dt-page` restates the two gutter tokens instead of taking `full`
...
`.dt-shot` is `overflow: hidden` because the iframe inside it renders at the
...
1. **The `.dt-mirror-stage:has([data-dt-before])` outline is the only place a
...
2. **`.dt-live-panel`'s `data-layout-ignore` is set in `live.js`, not here** —

A verdict that follows a resize handle. Drag the window and it changes
...
Each of the four leading issues goes through `point()`
...
the document: resolving against anything else finds a real element at the wrong
address. ⚠ A page-level finding gets no ring and no affordance; that used to be a
`keep()` helper here, existing only to be the else branch, and it is `point()`'s
job now.

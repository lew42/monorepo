...
`DesignTool.js` and calling `analyze()` inside a real Chromium instance. This
...
⚠ There is no `score` or `grade` field any more — `score.js` says why. A row keeps
its summary (counts, metrics) but drops

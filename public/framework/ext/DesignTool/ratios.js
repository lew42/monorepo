...
// Each node's direct children, by index. Three modules had hand-rolled this.
...
/* One node's subtree, flat. Nodes are preorder, so one forward pass carries
 * membership down — the cheapest way to ask a band about the CONTENT region
 * rather than about the app shell around it.
 * ⚠ NOT `under`: `rules.js` has a local `under()` severity helper and imports
 *   `under_scroller` from here, and a third meaning of the word one import line
 *   away is a collision nothing would report. */
export function inside(m, node){
	if (!node) return m.nodes;

	const inside = new Array(m.nodes.length).fill(false);
	inside[node.i] = true;
	for (const n of m.nodes) if (n.parent >= 0 && inside[n.parent]) inside[n.i] = true;

	return m.nodes.filter(n => inside[n.i]);
}

/* How much of the width the layout was GIVEN does its content actually COVER?
 * The prime objective as a number, and the one implementation both tiers read.
 * Why it is a union and not a span: knowledge/ideal-ranges.md, `width-used`.
 *
 * ⚠ COVERAGE, NOT A SPAN. A span from leftmost content to rightmost assumes the
 *   thing being measured — that what lies between is continuous — so a topbar with
 *   a logo left and an avatar right reads 100% and covers 5%.
 * ⚠ CLIPPED TO THE FRAME, or a 2640px code line inside a scrolling `<pre>` reports
 *   its unclipped extent and the answer comes back 208%.
 * ⚠ WHAT IS DRAWN, not what is nested: a box that merely HOLDS things covers the
 *   width without spending it. Text blocks AND leaves — leaves alone miss every
 *   paragraph holding an inline `<code>`, text alone misses a wall of image tiles.
 *   Overlap between the two is free, because a union merges it. */
export function width_used(m){
	const root = m.nodes[0];
	if (!root || !(root.w > 0)) return null;

	const parent = new Array(m.nodes.length).fill(false);
	m.nodes.forEach(n => { if (n.parent >= 0) parent[n.parent] = true; });

	const spans = m.nodes
		.filter(n => boxed(n) && n.w > 4 && n.h > 4 && (n.text || !parent[n.i]))
		.map(n => [Math.max(n.x, root.x), Math.min(n.x + n.w, root.x + root.w)])
		.filter(([a, b]) => b > a);

	// ⚠ `read.js`'s own floor, unchanged: too small a sample is nothing to measure,
	//   not a bad score. It lives here now so both tiers decline together — math
	//   answered 61% on a page taste declined to read at all.
	return spans.length < 6 ? null : covered(spans) / root.w;
}

// Total length these intervals cover, overlaps counted once.
function covered(spans){
	const sorted = [...spans].sort((a, b) => a[0] - b[0]);
	let total = 0, at = sorted[0][0], end = sorted[0][1];

	for (const [a, b] of sorted){
		if (a > end){ total += end - at; at = a; end = b; }
		else if (b > end) end = b;
	}

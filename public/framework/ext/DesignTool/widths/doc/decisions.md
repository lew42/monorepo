...
The four `library/` rows measure their own page directly, root `.dt-case-body`
...
- ⚠ **`frame()`'s `root` can silently measure the wrong document.** `DesignTool.js`'s
  ...
  file to fix — `frame()` ships in `../DesignTool.js` — named here because this is the

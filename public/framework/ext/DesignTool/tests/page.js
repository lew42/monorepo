...
const url_of = c => `/framework/ext/DesignTool/tests/${Page.slug(c.title)}/`;
...
export default new Page({
	...
	async run(width){
		...
		for (const c of cases){
			try {
				rows.push({ c, report: await frame(url_of(c), width, { root: ".dt-case-body" }) });
			} catch (error){
	...
	results(rows, width){
		...
		ui.table(
			["Case", "Expected", "Findings", "Detected", ""],
			scored.map(r => [
				() => a(r.c.title).attr("href", url_of(r.c)),
				expected(r.c, r.n_a),
				r.error ? "—" : census(r.report.counts),

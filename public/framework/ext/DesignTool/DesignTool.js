...
import { leading, metrics } from "./score.js";
...
/* The third tier, from the same door, and the only one that scores. `analyze()` says
 * what is BROKEN — a census, no grade (score.js says why). `rate()` says how GOOD it
 * is, which is the question a generator searches on. taste/readme.md. */
...
export function analyze(target = document.querySelector(".app") ?? document.body, opts = {}){
	...
	return {
		...
		/* ⚠ HOW MUCH OF THE PAGE THIS REPORT NEVER SAW. `probe.IGNORE` skips demo
		 * stages by policy, and measured across the site that is **39% of all
		 * page-widths audited** — every `ui/*` component page, most `styles/elements/*`
		 * and every `styles/layouts/*` gallery is more skipped than read. Those pages
		 * have been graded on their chrome since this tool existed and nothing said so.
		 * A caveat, not a fix: `taste/`'s `mostly_picture` is the same number. */
...
function siblings(issues, model){
	...
	for (const [key, flock] of flocks){
		...
		/* ⚠ `spot` IS THE EXEMPLAR — where to LOOK, as against where the fix goes. The
		 * attribution belongs on the parent, which is what `parent_fix()` is for; the
		 * location does not, because the parent of a page's direct children is the page
		 * container. Measured: 32 of 47 rings covering ≥60% of the viewport were
		 * roll-ups, pointing at boxes like 390×25731. `highlight.js` reads it. */
		out.push({
			...worst,
			sel: parent.sel, node: parent.i, path: parent.path, children: flock.length,
			spot: { path: worst.path, sel: worst.sel },
 ...
 * threw. DesignTool.css says the same thing for a frame someone else builds.
 *

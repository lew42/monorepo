...
- `browse(bands, tokens, options)` takes four options for a big catalogue — `cap` (how many cards before a *show more*), `facets` (rail rows read straight off a child page's own props, defaulting to *all*), `search` (extra props the box reads) and `rail: false` (for a caller with its own filter UI — skips the sticky rail, keeps `data-band="<group>"` on every heading so that caller can still scroll to one); a call with none renders byte for byte what it did: [`doc/method/browse.md`](./doc/method/browse.md).
- `browse()` band sizes matter, but a short band no longer stretches unboundedly — a card inside `.browse-band` caps at `28em` (2026-09-18); the heading sits outside its grid: [`doc/method/browse.md`](./doc/method/browse.md).

...
export default new Doc({
	...
	notes: "about tree panels fetched",
	...
	overview: [
		{
			...
			content(){
				...
				md("Pass `about` and the browser is **three** regions instead of two. It is called once per shown path with a view — or a promise of one — and its return fills the `about` panel. [About](/framework/ext/files/docs/about/) has the full contract and the capture trap.");
			},
		},
	],
	...
	content(){
		...
		md("Those are **real files on disk**, fetched. Not string literals in this page — so they can't drift, and if one is deleted the pane says so instead of quietly lying. [Fetched](/framework/ext/files/docs/fetched/) is the full argument.");
		...
		md("Nothing is saved. A `MemorySaver` means an arrangement lives as long as the page and every visit gets the seeded one — arranging here is exploring, not authoring. The axis is seeded too: a column below 640px, a row above, because a split holds its axis at every width. [Panels](/framework/ext/files/docs/panels/) has the whole argument, and what the flex columns it replaced were doing.");
		...
		md.details(import.meta, "readme.md", "Design record — why fetch, why a tree, and why the columns became panels");
	},
});

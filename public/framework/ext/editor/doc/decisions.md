...
[`Item`](../../core/Item/) tree, one [`Sortable`](/framework/ext/Draggable/) per node,
[`ext/layout`](/framework/ext/layout/)'s word vocabulary as the properties region, a
[`Saver`](/framework/ext/Saver/) holding the document — and a second, separate `Saver`
holding the shell, which is a [`Panel`](/framework/ext/Panel/) workspace. Four source files —
`blocks.js` (the palette), `History.js` (undo), `editor.css`, and `page.js` (the
widget and its doc page, at 330 lines the largest in the framework).
...
registry: [`doc/shell.md`](./shell.md).
...
which is what makes `/framework/ext/editor/` a route. [`ext/DesignTool`](/framework/ext/DesignTool/)'s
audit crawl lists that url, and a handful of `ai/` task pages and
[`ext/Panel/page.js`](/framework/ext/Panel/)'s "Next:" line link to it in prose — none of that
...
- **⚠ The `/framework/` wall card had a `preview()` override — do not bring it
  back without solving the sizing first.** It fetched the saved document and
  rendered a real `sketch()` of it at `zoom-25`, unstyled nesting and all (2026-08-16,
  found via `layout-hunt`'s audit and the `browsable` crawl — both independently
  flagged it). Once settled it measured correctly (thumb, inner and container
  rects all matched), but on a cold/first paint — every headless screenshot at
  both 1280 and 3440 reproduced it — the un-zoomed content painted at something
  close to natural size before the `zoom` recalculation caught up, spilling the
  `Section`/`Card`/`Text` tree (and the `Card` block's own `surface` box) down
  the page and shoving the card's title out from under it. `Page.css`'s
  `page-preview-thumb` clips by height, not width, and a thumb's content here has
  no fixed size of its own to clip against — `words: "flex v gap pad"` is a
  responsive layout, not a mockup with a natural width, so `zoom`'s "auto width
  against the containing block" resolution has nothing stable to lock onto while
  the wall's grid is still being laid out. Removed rather than patched: the card
  is now the plain default (icon, title, description), like every other
  extension on the wall. `.editor-thumb` (editor.css) and `sketch()` (page.js)
  went with it — no other caller.

## Open

- **`page.js` is 318 lines carrying three jobs** (widget, regions, doc page) —
  see [`doc/file/page.js.md`](./file/page.js.md) for the split this argues

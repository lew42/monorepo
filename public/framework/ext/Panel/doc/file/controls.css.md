...
**Renamed from `toolbar.css` on 2026-09-18, when the floating bar it was named
after was deleted** (`ai/2026-09-17/editor-select/`). Two things live here now,
and neither of them was ever a bar control:

- **`.panel-btn`** — one button box for every surface that draws this module's
  vocabulary: the rail (`properties.js`), a seam's hug/fill menu, the drag grip.
- **`.panel-bar`** — the strip across the top of a panel. It holds the drag grip
  and, on a leaf, the magnifier. It is out of flow, so a panel's content is the
  same size whether the strip is showing or not.

Everything below still describes that strip and still holds; only what rides on
it has changed. `.panel-gap` (which pushed `close` to the far end of the row) is
deleted with the row it pushed in.

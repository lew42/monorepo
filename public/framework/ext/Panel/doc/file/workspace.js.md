...
The two doors, the redraw, and the recursive view: `panel(seed)` and
`workspace(options)`, and the `view()` that walks a `Panel` tree into DOM. It is
the *assembly* file — the bar is `toolbar.js`, the divider is `grip.js`, the drag
is `PanelDrag.js`, the `random` verb is `random.js`, and four neighbours carry
what this file used to hold inline: `vocab.js`, `focus.js`, `overlays.js`,
`paint.js`. This is the only file that calls them all.
...
layout over the real one. See [Decisions](/framework/ext/Panel/doc/decisions/).
...
A leaf's body gets one further call, `overlays(item, $body)`, on the line after
the body is created — ⚠ **there**, because everything it binds binds to that
element, which `paint()` empties but never replaces. The two surfaces that are
not on a body stay here beside what they attach to: `edges(item, $panel)` and
`insert_bar(item, $items)`.
...
## The vocabulary and the tool flags are resolved, not held

`root.templates` and `root.tools` are instance properties on the root panel,
written once by `workspace({ templates, tools })` and read by walking up —
[`vocab.js`](/framework/ext/Panel/files/) is the whole of that story. This file
only *uses* the resolvers: `vocab(item)` for the `T` entries, `offer(item)` for
the names it lists, `tools(item)` for the surfaces `view()` gates, and
`standard(item)` for the two things only the site's own vocabulary is offered.
...
Three ideas that were inline here are their own files now, each read by this one
and none reading it back — the 2026-08-17 pass that took this file from 365 lines
back to 198:

- [`paint.js`](/framework/ext/Panel/files/) — `paint()` (whose content wins, the
  `text_commit` that runs first, the `console.warn` for a name no vocabulary
  has), `show()`, the `views` WeakMap and `repaint()` (the DOM of a panel you are
  not inside — which is how the bar's `sow` verb lands a single-leaf seed that
  moves no children), and `repaint_mirrors()`, the live-duplicate walk `mount()`
  wires to `change`.
- [`focus.js`](/framework/ext/Panel/files/) — `focused()`, `focus()`, the
  `panel-focus`/`panel-unfocus` document events, and `selection(root, $root)`,
  which installs the three ways a selection is lost. The whole model:
  [Focus, and the panel that reads it](/framework/ext/Panel/doc/focus/).
- [`overlays.js`](/framework/ext/Panel/files/) — the four surfaces bound to a
  leaf's body, and the disposer registry `draw()` drains.

⚠ **`roll()` did not move.** Its `random` branch raises the same module-scope
`drawing` flag `draw()` reads, and a flag with exactly two users belongs in the
file that holds both — an import for it in either direction would be the ring
this module has avoided all along.
...
`paint()` is `paint.js`'s now, and exported — so what keeps `roll()` here is the
`drawing` flag its `random` branch shares with `draw()`.
...
3. **`scatter()` and `resolve()` were the `random` verb living beside the doors
   ...
   reason above. The room that move made was refilled within a day — by focus,
   four body overlays and `repaint()` — which is what the 2026-08-17 pass
   answered by naming all three (above), 365 lines back down to 198.
   *(medium — done)*

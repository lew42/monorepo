...
div.c("layout", () => {
    ...
    layout.context($wall, $sel => toggle($sel, "checkered"));
		...
		md("`layout.bar()` takes a **`View`**, a bare **`Element`**, or a **`Page`** — `view_of()` is the whole difference between them. Hand it this page and the same toolbar steers the page's own shape words: **`standard`** (what a page renders as by default), **`sheet`**, **`pad`**, **`full`**, plus **`fill`**, **`flow`** and a `--measure` knob. Pick one — *this* page re-arranges under you.");

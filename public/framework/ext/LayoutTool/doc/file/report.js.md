A report as a view. `report(data)` is the whole panel — grade, metrics, ranked
...
The group heading and each instance row are `aim()`ed
(`highlight.js`): hover to ring the element on the page, click
to keep the ring and scroll to it. The heading points at the exemplar — the
parent a sibling roll-up landed on, which is the box you would actually edit —
and each row at its own finding.
...
2. **`band()` (used for the score's color) and `SEV` (used for a finding's
   ...
   saying so explicitly, since both end up as `lt-ok`/`lt-warn`/`lt-bad` /
   `lt-sev-*` classes a reader could otherwise assume are the same scale.
   *(simple, speculative.)*

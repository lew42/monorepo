...
export const BREAKS = [{
	/* ⚠ `base` first, and this case is why it exists. Unbounding `--measure: 52em`
	   changed NOTHING at any width, because 52em is already ~117 characters a line
	   here — the subject was broken before the break. A pair only means something if
	   the left-hand side is actually good, so the baseline binds it properly. */
	id: "unbound the prose",
	band: "measure",
	why: "a reading column with no ceiling runs 117 characters a line at 3440",
	...
	/* ⚠ EXPECTED TO FAIL, and it is a finding rather than a bad case. A layout's gap
	   ...
	   container. Eleven scrambled spec gaps never reach the set covering four fifths
	   of the page's, so `scale` cannot be moved from the layout at all. It is a band
	   about a design system, not about an arrangement. */
...
export const SUBJECTS = ["document", "docs", "dashboard", "landing", "gallery"];
...
/* Bind the reading column — and INSTALL one where the subject has none. Unbounding a
   `--measure: 52em` that already runs 117 characters a line changes nothing, so a
...
export function judge(before, after, brk){
	...
	const was = credit(before, ids), now = credit(after, ids);

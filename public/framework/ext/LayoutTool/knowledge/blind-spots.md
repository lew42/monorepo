...
[Scroller in a wrapping row](/framework/ext/LayoutTool/library/bad/scroller-in-a-wrapping-row/).
...
- A **table** is invisible to it. Only one column of
  [Wide table](/framework/ext/LayoutTool/library/wide-table/) holds text over 20
...
`width_used` in the metrics carries the same limitation and should be read as
*where the prose is*, not *where the layout is*.
...
[Pixel padding](/framework/ext/LayoutTool/library/bad/pixel-padding/): low at
...
Fixed in `LayoutTool.css` with `iframe[data-layout-ignore] { max-width: none }`

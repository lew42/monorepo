...
const url_of = title => `/framework/ext/LayoutTool/library/${Page.slug(title)}/`;
...
export default new Page({
	...
	content(){
		md("**Eleven arrangements and ten ways to get them wrong.** Each card opens a page holding the "
			+ "declaration, the pattern rendered live at the width you are reading at, a score that follows "
		...
		md("**A finding here is not automatically a fault.** A reading column at 3440 trips `dead-space` "
			...
			+ "Where the rules came from: [thresholds](/framework/ext/LayoutTool/knowledge/thresholds/) · "
			+ "what never to flag: [false positives](/framework/ext/LayoutTool/knowledge/false-positives/) · "
			+ "does it hold at every width: [responsiveness](/framework/ext/LayoutTool/knowledge/responsiveness/).\n\n"
			+ "This catalog answers *what to write*. [Test corpus](/framework/ext/LayoutTool/tests/) answers a "

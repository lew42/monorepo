...
import { each_child, boxed, text_bounds } from "./ratios.js";
...
export const polish = [
	...
	/* Mike's card: 20px of padding on a 1000px box "looks off", and it does —
	 ...
	 *
	 * So this rule takes the wide window that `alignment` cannot afford, by
	 * restricting itself to the one relationship where it is safe. */
	rule("heading-offset", "alignment", "A heading doesn't line up with its own text", m => {
		const flow = m.nodes
			.filter(n => n.text && n.w > 40 && !n.display.startsWith("inline"))
		...
		for (let i = 0; i < flow.length - 1; i++){
			const head = flow[i];
			if (!level(head)) continue;

			/* ⚠ A heading following a heading counts. The reported case was a page
			 * TITLE against the `h2` inside a padded wrapper — skipping
			 * heading-over-heading as "hierarchy's problem" made the rule miss the
			 * exact misalignment it was written for. */
			const next = flow[i + 1];
			if (next.y < head.y + head.h - 2) continue;       // side by side, not above/below
			if (next.y - (head.y + head.h) > head.h * 3) continue;   // too far apart to read as a pair

			const off = Math.abs(next.x - head.x);
			if (off < 2 || off > 96) continue;                // 96px+ is a deliberate indent

			found.push(issue(next, off > 12 ? "med" : "low", off,
				`starts ${Math.round(off)}px ${next.x > head.x ? "right" : "left"} of the `
				+ `"${head.sel}" above it — a heading and its text are one column`,
				{ sel: next.sel, decl: "padding-inline: 0; margin-inline: 0" }));
		}

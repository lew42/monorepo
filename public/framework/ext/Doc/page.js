...
// …or no subject at all: notes: and files: document a module of loose functions`);
		...
		md("The list is hand-typed for the same reason `methods` is, and for one more: `directory.json` is gitignored, so a crawler-driven tab would be blank in production and nobody who could fix it would ever see it break. [Why the file list is declared](/framework/ext/Doc/doc/files/).");
